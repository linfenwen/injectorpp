...
FunctionObject::FunctionObject(PSYMBOL_INFO sym, STACKFRAME64 frame) : m_isReturnObj(false)
{
    m_symbol = sym;
...
FunctionObject::FunctionObject(PSYMBOL_INFO sym) : m_isReturnObj(true)
{
    m_symbol = sym;
    m_hProcess = GetCurrentProcess();
    LoadType(sym);
...
void FunctionObject::LoadValue(STACKFRAME64 frame, ULONG64 addr)
{
    ...
    if (m_tag == SymTagEnum)
    {
        ...
        // be an 1-byte value and therefore we take 3 additional
    ...
    case VT_I4:
    {
        m_value.intVal = *(int*)mem;
        break;
    }
    case VT_R4:
    {
        m_value.fltVal = *(FLOAT*)mem;
        break;
    }
    case VT_BOOL:
    {
    ...
    case VT_I1:
    {
        m_value.intVal = *(signed __int8*)mem;
        break;
    }
    case VT_I1 | 0x8000:
    case VT_I2:
